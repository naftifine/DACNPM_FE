    ...
    const handleSubmitOrder = async () => {
        ...
        const orderData = {
            data: {
                ...
                userid: user.userid, // ID người mua
                quantity: quantity, // Số lượng sản phẩm
                totalprice: totalPrice, // Tổng tiền bao gồm phí vận chuyển
                shippingfee: shippingFee, // Phí vận chuyển
                shippingAddress: document.querySelector('input[placeholder="Tỉnh/Thành phố, Quận/Huyện, Phường/Xã"]').value.trim()+' '+document.querySelector('input[placeholder="Tên đường, Tòa nhà, Số nhà"]').value.trim(),
                buyerNote: document.querySelector('textarea[placeholder="Lời nhắn cho người bán"]').value.trim(),
            },
            
        };
        try {
            const response = await axiosInstance.post("http://localhost:3001/order/addOrder", orderData);
            if (response.data.success) {
                alert("Đặt hàng thành công!");
                navigate("/orderlist"); // Chuyển hướng tới trang danh sách đơn hàng
            } else {
